...
# Initial health check by getting the status code from Harbor ping endpoint
STATUS_CODE=$(curl -k -o /dev/null -s -w "%{http_code}\n" https://${HARBOR_HOSTNAME}:${HARBOR_PORT}/api/v2.0/ping)
...
if [ "$STATUS_CODE" -eq 200 ]; then
  curl -k -s -u "admin:${CURRENT_PASSWORD}" -X GET "https://${HARBOR_HOSTNAME}:${HARBOR_PORT}/api/v2.0/systeminfo"
