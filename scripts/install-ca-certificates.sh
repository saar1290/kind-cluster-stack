...
if [ ! -f /etc/ssl/certs/ca.pem ]; then
    echo $SUDO | sudo -S cp ${KIND_SSL_CERTS_DIR}/ca.crt /usr/local/share/ca-certificates/kind-cluster-ca.crt
    echo $SUDO | sudo -S update-ca-certificates
    ...
    echo $SUDO | sudo -S rm -f /etc/ssl/certs/ca.pem
    echo $SUDO | sudo -S cp ${KIND_SSL_CERTS_DIR}/ca.crt /usr/local/share/ca-certificates/kind-cluster-ca.crt
    echo $SUDO | sudo -S update-ca-certificates
fi